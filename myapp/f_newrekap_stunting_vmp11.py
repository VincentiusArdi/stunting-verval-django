...
async def newrekap_stunting_vmp11(params):
    ...
    query = sql.SQL("""
		select src.kki, src.nik, src.nama, src.status_hamil, src.kesertaan_kb_modern, src.sumber_air_minum, src.memiliki_tempat_bab, src.status_keluarga, src.kesejahteraan_prioritas, src.sasaran_krs, src.nama_istri, src.id_provinsi, src.id_kabupaten, src.id_kecamatan, src.id_kelurahan, src.id_rw, src.id_rt, src.status_pus, src.resiko_stunting, src.balita, src.baduta, src.nik_tidak_wajar, src.sasaran_genting, src.sudah_diukur, src.tanggal_kunjungan, src.status_pengukuran, src.id_frm
		from (
			select distinct pk.kki::text AS kki,
				...
				COALESCE((case when COALESCE(COALESCE(pk.status_pus, pk.status_pus_manual), 2) = 1 and coalesce(pk.siga_kesertaan_berkb, pk.kesertaan_berkb) in ('Sedang', 'Pernah')
					then (case when COALESCE(pk.siga_metode_kontrasepsi, pk.metode_kontrasepsi) = 'BUKAN PESERTA KB' then '0'
						when COALESCE(pk.siga_metode_kontrasepsi, pk.metode_kontrasepsi) = 'MOW' then '1'
						when COALESCE(pk.siga_metode_kontrasepsi, pk.metode_kontrasepsi) = 'MOP' then '2'
						when COALESCE(pk.siga_metode_kontrasepsi, pk.metode_kontrasepsi) = 'IUD' then '3'
						when COALESCE(pk.siga_metode_kontrasepsi, pk.metode_kontrasepsi) = 'IMPLANT' then '4'
						when COALESCE(pk.siga_metode_kontrasepsi, pk.metode_kontrasepsi) = 'SUNTIKAN' then '5'
						when COALESCE(pk.siga_metode_kontrasepsi, pk.metode_kontrasepsi) = 'PIL' then '6'
						when COALESCE(pk.siga_metode_kontrasepsi, pk.metode_kontrasepsi) = 'KONDOM' then '7'
						when COALESCE(pk.siga_metode_kontrasepsi, pk.metode_kontrasepsi) = 'MAL' then '8'
						when COALESCE(pk.siga_metode_kontrasepsi, pk.metode_kontrasepsi) = 'TRADISIONAL' then '9'
						when COALESCE(pk.siga_metode_kontrasepsi, pk.metode_kontrasepsi) = 'TIDAK BERLAKU' then '99'
						end)
					end)::text, '0') as kesertaan_kb_modern,
				pk.sumber_air_minum::varchar as sumber_air_minum,
				pk.memiliki_tempat_bab::varchar as memiliki_tempat_bab,
				pk.status_keluarga::varchar,
				pk.kesejahteraan_prioritas::varchar as kesejahteraan_prioritas,
				null::boolean as status_verval,
				pk.nama_istri::text as nama_istri,
				...
				pk.id_provinsi as id_provinsi,
				pk.id_kabupaten as id_kabupaten,
				pk.id_kecamatan as id_kecamatan,
				pk.id_kelurahan as id_kelurahan,
				pk.id_rw as id_rw,
				pk.id_rt as id_rt,
				(case when to_char(age({v_tanggal}, pk.baduta_balita), 'YYYY'::text)::integer < 5
					or COALESCE(COALESCE(pk.status_pus, pk.status_pus_manual), 2) = 1
					or ((case when pk.status_hamil = true and ((EXTRACT(days FROM ({v_tanggal} - pk.updated_date)) / 7)::int + coalesce(pk.usia_kehamilan,0)::int) <= 40 then pk.status_hamil else false end)::boolean = true and COALESCE(COALESCE(pk.status_pus, pk.status_pus_manual), 2) = 1)
				then 1 else 2 end)::int as resiko_stunting,
				pk.nik_tidak_wajar,
	--            case when to_char(age({v_tanggal}, pk.baduta_balita), 'YYYY'::text)::integer < 5 then 1 else 2 end as flag_genting,
			...
			FROM 
				(select a.kki
					...
					,case when a.kesertaan_berkb = '1' then 'Sedang'
						when a.kesertaan_berkb = '2' then 'Pernah'
						when a.kesertaan_berkb = '3' then 'Tidak Pernah'
					end as kesertaan_berkb
					,a.created_date
					...
					,case when a.siga_kesertaan_berkb = '1' then 'Sedang' end as siga_kesertaan_berkb
					,a.siga_ada_suami
					,a.updated_date
					,a.sumber_air_minum
					,a.memiliki_tempat_bab
					,a.siga_created_date
					,ibrl.baduta_balita
					...
					,a.sudah_diukur
					,a.tanggal_kunjungan_baduta as tanggal_kunjungan
					,a.status_pengukuran
					,(case
					...
					,(case
						when dkt.anak_2terakhir - dkt.anak_terakhir < 2
						and coalesce(coalesce((case when to_char(age({v_tanggal}, ibrl.tgl_lahir_istri), 'YYYY'::text)::int between 10 AND 49 then 1 else null end), (case when to_char(age({v_tanggal}, ibrl.tgl_lahir_manual), 'YYYY'::text)::int between 10 AND 49 THEN 1 else null end)), 2) = 1
					then 1
						when dkt.anak_2terakhir - dkt.anak_terakhir >= 2
						and coalesce(coalesce((case when to_char(age({v_tanggal}, ibrl.tgl_lahir_istri), 'YYYY'::text)::int between 10 AND 49 then 1 else null end), (case when to_char(age({v_tanggal}, ibrl.tgl_lahir_manual), 'YYYY'::text)::int between 10 AND 49 THEN 1 else null end)), 2) = 1
