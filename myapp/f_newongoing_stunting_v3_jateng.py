...
async def stunting_v3_jateng(params):
	...
	from (
		select distinct a.kki::text AS kki,
			...
			(case when a.status_keluarga = 6 and (a.created_date < '2025-06-16' or a.updated_date < '2025-06-16') then 1 else a.status_keluarga end)::varchar as status_keluarga,
			...
			a.dapat_bansos_genting,
			a.sudah_diukur,
			a.tanggal_kunjungan,
			a.status_pengukuran,
			a.tanggal_lahir_anak_terakhir,
			case when a.sts_paud = 1 then true end as sts_paud,
			a.id_frm,
			a.no_kk,
			a.jarak_sam,
			a.memiliki_aset,
			...
			a.luas_rumah
	    from verval_jateng.stunting_head a
